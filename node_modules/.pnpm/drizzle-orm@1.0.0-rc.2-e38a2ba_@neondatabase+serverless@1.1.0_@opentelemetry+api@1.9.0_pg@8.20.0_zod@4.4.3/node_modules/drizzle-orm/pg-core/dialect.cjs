	...
	*/
	...
	buildRqbColumn(table, column, key, inJson) {
		if ((0, __entity_ts.is)(column, __column_ts.Column)) {
			...
			return __sql_sql_ts.sql`${inJson && column.jsonSelectIdentifier ? column.jsonSelectIdentifier(name, __sql_sql_ts.sql, column.dimensions) : this.codecs.apply(column, inJson ? "castInJson" : "cast", name)} as ${__sql_sql_ts.sql.identifier(key)}`;
		}
	...
	unwrapAllColumns = (table, selection, inJson) => {
		return __sql_sql_ts.sql.join(Object.entries(table[__table_ts.TableColumns]).map(([k, v]) => {
			selection.push((0, __entity_ts.is)(v, __column_ts.Column) ? {
				key: k,
	...
	buildColumns = (table, selection, inJson, config) => config?.columns ? (() => {
		...
		for (const [k, v] of entries) {
			...
			if (v) {
				...
				selection.push((0, __entity_ts.is)(column, __column_ts.Column) ? {
					key: k,
