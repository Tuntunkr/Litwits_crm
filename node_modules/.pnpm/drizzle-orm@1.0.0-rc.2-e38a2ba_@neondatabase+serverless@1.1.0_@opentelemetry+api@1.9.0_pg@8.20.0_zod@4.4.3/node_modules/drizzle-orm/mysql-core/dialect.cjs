...
var MySqlDialect = class {
	...
	*/
	...
	_buildRelationalQueryWithoutLateralSubqueries({ fullSchema, schema, tableNamesMap, table, tableConfig, queryConfig: config, tableAlias, nestedQueryRelation, joinOn }) {
		...
		where = (0, __sql_expressions_index_ts.and)(joinOn, where);
		if (nestedQueryRelation) {
			let field = __sql_sql_ts.sql`json_array(${__sql_sql_ts.sql.join(selection.map(({ field }) => (0, __entity_ts.is)(field, require_mysql_core_columns_common.MySqlColumn) ? __sql_sql_ts.sql.identifier(field.name) : (0, __entity_ts.is)(field, __sql_sql_ts.SQL.Aliased) ? field.sql : field), __sql_sql_ts.sql`, `)})`;
